...
public class AdminHeader extends HttpServlet {
	...
	private void classestList(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// get subjects from db util
		List<Class> classes = dAO.getClasses();
	...
	private void login(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (username.toLowerCase().equals("simplilearn") && password.toLowerCase().equals("password")) {
	...
	private void classStudentsList(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// get subjects from db util
		List<Student> students = dAO.loadClassStudents(classId);
	...
	private boolean getCookies(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		boolean check = false;
		Cookie[] cookies = request.getCookies() == null ? new Cookie[100]: request.getCookies();
		// Find the cookie of interest in arrays of cookies
		for (Cookie cookie : cookies) {
			if(cookie!=null)
			 
			if (cookie.getName().equals("simplilearn") && cookie.getValue().equals("password")) {
				check = true;
				break;
			} 
